...
runCohortMethod <- function(connectionDetails,
  ...
  subset <- subset[!is.na(subset$strataFile) & subset$strataFile != "", ]
  if (nrow(subset) > 0) {
    subset <- split(subset, seq(nrow(subset)))
    cluster <- ParallelLogger::makeCluster(min(3, maxCores))
    ParallelLogger::clusterApply(
      cluster, subset, computeCovariateBalance,
      cmOutputFolder = cmOutputFolder, balanceFolder = balanceFolder
    )
...
computeCovariateBalance <- function(row, cmOutputFolder, balanceFolder) {
  outputFileName <- file.path(
    ...
    sprintf("bal_t%s_c%s_o%s_a%s.rds",
            row$targetId, row$comparatorId,
            row$outcomeId, row$analysisId)
  )
  if (!file.exists(outputFileName)) {
    ...
    balance <- CohortMethod::computeCovariateBalance(
      population = strata, cohortMethodData = cohortMethodData
    )
...
createTcos <- function(outputFolder) {
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "HIRA")
  ...
  createTco <- function(i) {
    ...
    outcomeIds <- as.character(
      tcosOfInterest$outcomeIds[
    ...
    outcomeIds <- c(
      ...
      allControls$outcomeId[
        allControls$targetId == targetId &
        allControls$comparatorId == comparatorId
      ]
    )
    excludeConceptIds <- as.character(
      tcosOfInterest$excludedCovariateConceptIds[
        tcosOfInterest$targetId == targetId &
        tcosOfInterest$comparatorId == comparatorId
      ]
    )
    ...
    includeConceptIds <- as.character(
      tcosOfInterest$includedCovariateConceptIds[
        tcosOfInterest$targetId == targetId &
        tcosOfInterest$comparatorId == comparatorId
      ]
    )
  ...
  tcosList <- lapply(seq_len(nrow(tcs)), createTco)
  return(tcosList)
}
...
getOutcomesOfInterest <- function() {
  ...
  outcomeIds <- do.call(
    "c",
    strsplit(as.character(tcosOfInterest$outcomeIds), split = ";")
  )
...
getAllControls <- function(outputFolder) {
  ...
  if (file.exists(allControlsFile)) {
    ...
    pathToCsv <- system.file("settings", "NegativeControls.csv", package = "HIRA")
